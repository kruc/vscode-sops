...
name: release
description: >
  Releases the vscode-sops extension end-to-end: determines the next version from the bump history,
  updates CHANGELOG.md and package.json/package-lock.json, builds via `vscode:prepublish`, publishes
  to VS Code Marketplace (vsce) and Open VSX Registry (ovsx), tags and pushes the release commit, and
  creates a GitHub Release with a changelog. Handles token expiration with re-generation guidance and
  stops on any build or type error, showing issues and suggestions before asking to continue.
  Use when the user asks to "release", "publish", or "bump and publish" the extension.
---
